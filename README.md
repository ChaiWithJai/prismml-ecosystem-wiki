...
Community documentation for PrismML's Bonsai model family and the local-inference ecosystem around it. Live at [yourwildcard.ai](https://yourwildcard.ai).
...
Facts come from official sources. A product claim appears only when it links to an official source with the date it was checked; claims that exist only on social media stay labeled as unconfirmed. Recipes that nobody has run on hardware say "not verified" and carry an acceptance test. See the [contributor guide](https://yourwildcard.ai/docs/contribute/contributor-guide) to add a page, a device report, or a correction.

## Sources

The [1-bit Bonsai 8B whitepaper](https://yourwildcard.ai/whitepaper/1-bit-bonsai-8b-whitepaper.pdf) (PrismML), the official PrismML model cards on Hugging Face, and the upstream documentation of every runtime covered. Bonsai models are Apache 2.0.
